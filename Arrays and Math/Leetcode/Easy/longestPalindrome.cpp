...
//We use a character-integer paired map to store the frequency of every character in the string. A bool is initialised to false to keep track if an odd frequency was added to the length, once we add an odd length (say size 7), we know that the even part (of size 6) will be in the two extreme sides of the palindrome distributed evenly, and the left over character will be placed in the middle. After this, we can no longer add odd lengths, therefore, if we encounter a chracter with odd frequency again, we only take the even part out of it and distribute it evenly to form the palindrome.
